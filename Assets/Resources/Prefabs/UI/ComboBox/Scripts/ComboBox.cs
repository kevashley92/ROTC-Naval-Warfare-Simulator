...
public class ComboBox : MonoBehaviour {
   ...
   public Sprite ComboItemImage;
	public Action<int> OnSelectionChanged;
	public Action<int> OnItemSelected;
	public Color32 textColour = new Color32 (0, 0, 0, 255);
	public Color32 disabledTextColour = new Color32 (174, 174, 174, 255);
	public ColorBlock scrollButtonColour = ColorBlock.defaultColorBlock;
	public bool scrollBackgroundImageEnabled;
	public Sprite scrollBackgroundImage;
	[SerializeField]
	private bool
		_interactable = true;
	...
	public bool Interactable {
		...
		set {
			...
			if (!Application.isPlaying)
				return;
			if (!_interactable && overlayGO != null && overlayGO.activeSelf)
	...
	private void recursiveInit (GameObject obj) {
		if (initialised)
			return;
		bool on = obj.activeSelf;
		if (!on)
			obj.SetActive (true);
		if (!gameObject.activeInHierarchy)
			recursiveInit (obj.transform.parent.gameObject);
		else
			InitControl ();
		if (!on)
			obj.SetActive (false);
	...
	public void OnEnable () {
		if (!string.IsNullOrEmpty (language) && !language.Equals (LanguageManager.instance.Language)) {
			language = LanguageManager.instance.Language;
			Refresh ();
		}
	}
	...
	public void AddItems (params object[] list) {
		...
		foreach (var obj in list) {
			...
			if (obj is LocalizedItem) {
				var item = new ComboBoxItem (null, null, false, null, (LocalizedItem)obj);
				cbItems.Add (item);
				continue;
			}
	...
	public void CreateControl () {
		...
		comboButtonImage.color = new Color (0, 0, 0, 0);
		...
		comboButtonColors.fadeDuration = 10.1f;
	...
	private void InitControl () {
		...
		if (scrollBackgroundImageEnabled)
			scrollPanelImage.sprite = scrollBackgroundImage;
		else
	...
	private void Refresh () {
		...
		for (var i = 0; i < Items.Length; i++) {
			...
			itemTransform.GetComponent<Image> ().sprite = ComboItemImage;
			var itemText = itemTransform.Find ("Text").GetComponent<Text> ();
			itemText.text = item.LocalItem != null ? LanguageManager.instance.getString (item.LocalItem.file, item.LocalItem.label) : item.Caption;
			if (item.IsDisabled)
				itemText.color = disabledTextColour;
			else
				itemText.color = textColour;
			...
			if (scrollBackgroundImageEnabled) {
				...
				itemTransform.GetComponent<Image> ().color = Color.clear;
			} else
				itemButton.colors = scrollButtonColour;
	...
	public void RefreshSelected () {
		...
		comboTextRectTransform.GetComponent<Text> ().text = item != null ? (item.LocalItem != null ? LanguageManager.instance.getString (item.LocalItem.file, item.LocalItem.label) : item.Caption) : "";
		if (!Application.isPlaying)
			return;
		var i = 0;
